...
|---|---|---|---|
| v25 | Jun 14, 2026 | Claude | **Playful Minimalist redesign — all pages.** Dibuat `DecorativeBlobs` shared component. Semua halaman dashboard diredesign: gradient header cards (teal-to-emerald), decorative blur blobs, playful card styling (shadow + rounded + subtle border). **Transactions:** card container, category icon di tiap item. **Analytics:** gradient header, playful SectionCard + TotalsTile. **Wallets:** gradient header card, playful card styling. **Settings:** gradient header, playful sections. **Budget:** amber gradient header. **Login:** blobs + gradient logo icon. **Landing:** gradient buttons + gradient headings + blobs + playful cards. **Transaction detail:** playful hero card + detail card. **Quick Entry:** rounded tabs + gradient submit. **Nav sidebar:** gradient logo text. **WalletCard/BudgetCard:** playful styling. |
...
### AI Insights (Sub-phase 3.2 — Done)

| Task | Status | Notes |
|---|---|---|
| Migration 011: `ai_insights` (user_id, period_key, insights jsonb, model, RLS read-only) | ✅ | Applied via Supabase MCP |
| Inngest job: daily-insights (07:00 WIB) | ✅ | `lib/inngest/functions/insights-generate.ts` — cron `0 0 * * *`, concurrency 5, skip user no-activity, idempotent via period_key unique |
| Gemini prompt untuk insights Bahasa Indonesia | ✅ | `lib/ai/insights.ts` — buildInsightsPrompt + parseInsights (hardened: cap 3, max 200 chars, strip markdown fences) |
| Cache insights di database (24 jam) | ✅ | period_key = `YYYY-MM-DD` WIB; API returns most recent ≤ today (fallback ke kemarin kalau cron belum jalan) |
| API: GET /api/insights | ✅ | Auth + rate-limit (30/min), 6 route tests |
| Hook: use-insights | ✅ | `lib/hooks/use-insights.ts` — staleTime 1h |
...
| 13 | CIMB parser — fixture nyata + display-name | ⏳ | Parker Phase 2 |
| 14 | PARSER_GUIDE update — BNI/BRI/CIMB section | ⏳ | Parker Phase 2 |
| 15 | Pen-test manual (IDOR + auth bypass) → bump v1.0.0 | ⏳ | Final sweep sebelum public |
...
| Tanggal | Bug | Fix |
|---|---|---|
| Jun 12, 2026 | Login menampilkan dump ~10 transaksi lama (backfill tak diinginkan) | ROOT CAUSE: callback login jalankan `syncUserGmail` (incremental dari cursor tersimpan). Saat background sync mati ~2 minggu (bug service-role key + watch tak terdaftar), cursor beku di historyId ~26 Mei. Begitu sync diperbaiki, run sukses pertama menarik SEMUA email sejak cursor beku → 2 burst (11 baris @ 16:54, 10 baris @ 09:15). Bukan "email pra-connect" tapi backlog pasca-connect yang menumpuk. FIX: **forward-only at login** — callback & reconnect kini panggil `anchorGmailCursor()` (set `gmail_sync_token` = historyId sekarang TANPA impor) bukan `syncUserGmail`. Login tak pernah tarik backlog; capture real-time via background sync (cron + push). Tombol "Sync sekarang" tetap catch-up |
| Jun 12, 2026 | BCA parser ambil nama "bleed" ke field berikutnya (mis. "Source of Fund") | ROOT CAUSE: email BCA berupa HTML; `stripHtmlTags` collapse SEMUA whitespace (termasuk newline) jadi satu baris, jadi capture `[^\n]+` menyerap seluruh sisa string. FIX: ekstraksi nilai berhenti di label field berikutnya (`NEXT_LABEL` lookahead) bukan di newline — bekerja untuk body 1-baris maupun multi-baris. Reference No. di-capture token tunggal (anti-bleed "Please save…"), date+time dipisah agar jam tidak hilang. +4 test flattened-body |
| Jun 12, 2026 | Transfer dari Gmail terinput 2× (transaksi & saldo dobel) | ROOT CAUSE: migrasi `009_add_email_unique_constraint` **tidak pernah ter-apply ke prod** (drift — file 009 dilewat padahal 010 masuk), jadi dedup `(user_id, raw_email_id)` tak ada constraint pendukung. Dua sync run paralel (Pub/Sub at-least-once + race window di concurrency guard) fetch email yang sama & insert dua-duanya. `sync.ts` sudah benar handle `23505→skipped`; yang hilang cuma index-nya. FIX: apply unique index ke prod + soft-delete baris duplikat + koreksi saldo wallet (+1.000). Tidak ada perubahan kode. ⚠️ Audit drift migrasi lain |
